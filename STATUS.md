...
- **Core boundary scaffold** (`internal/core/`): first-pass client-facing facade over the legacy service/store stack.
  ...
  - Core read APIs now wrap the existing implementation for:
    ...
    - aggregate view-task collection (`CollectViewTasks`)
    - project list/query (`ListProjects`, `QueryProjects`)
    - weekly review (`WeeklyReview`)
    - Markdown import/export (`ImportMarkdown`, `ExportMarkdown`)
  - Core mutation/session APIs now wrap the existing implementation for:
    - inbox capture (`CaptureToInbox`)
    - patch-based task/project updates (`UpdateTask`, `UpdateProject`)
    - ID-first archive/restore/trash (`ArchiveTask`, `RestoreTask`, `TrashTask`, `RestoreTrashedTask`)
    - stable-ID move/refile operations (`MoveTaskToList`, `MoveTaskToProject`)
    - Process Inbox session lifecycle, draft mutation, and typed final decisions (`StartInboxSession`, `GetInboxSession`, `UpdateInboxDraft`, `CommitInboxDecision`, `SkipInboxItem`, `DiscardInboxSession`)
    - project/subgroup structure operations (`GetProject`, `CreateProject`, `CreateSubgroup`, `RenameSubgroup`, `DeleteSubgroup`, `AddProjectTask`, `MoveTaskToSubgroup`, `ReorderProjectTask`)
...
- TUI now holds both the legacy `*service.Service` and the new `*core.Core` during the extraction. Migrated non-project paths already using core APIs include: task detail save for non-project tasks; archive/trash/restore in view-results and weekly review for non-project tasks; inbox/actions direct done/cancel/archive/trash actions for non-project tasks; in-place waiting/someday transitions for single-actions tasks; the first non-Process-Inbox refile/move flows (refile to list, picker-based move to project, inbox waiting/someday routing); Process Inbox route movement after working-copy persistence back to inbox; Process Inbox session start/advance lifecycle, draft mutation, typed final decisions, and summary/progress ownership; aggregate-surface mutation handlers in view results and weekly review; the remaining raw-payload trash undo paths now hidden behind a core bridge; project-detail reorder through a stable-ID core API; inbox add through `core.CaptureToInbox(...)`; project creation through `core.CreateProject(...)`; project list loading through `core.ListProjects()`; weekly review loading through `core.WeeklyReview(...)`; and aggregate view task collection through `core.CollectViewTasks(...)`. Project-detail operations now using core APIs include subgroup create/rename/delete, add-project-task, move-task-between-subgroups, reorder-within-subgroup, project task done/cancel/archive/trash, in-place waiting/someday, project task detail save, and project metadata edit save. Remaining legacy areas are now much narrower and mostly presentation/state-model oriented: Process Inbox step-navigation/presentation ownership and filename-oriented project read/state in the TUI pending the separate ID-first follow-up.
...
- **Read-side service-boundary cleanup landed** — TUI project list loading now uses `core.ListProjects()`, weekly review loading uses `core.WeeklyReview(...)`, and aggregate task collection for views uses `core.CollectViewTasks(...)`. Project detail and project edit loading also no longer depend on the legacy service layer, though they still use the current filename-oriented TUI/store read model pending the separate ID-first state follow-up.
- **Final small mutation cleanup landed** — The last obvious direct TUI mutation entry points now use the core boundary too: inbox add routes through `core.CaptureToInbox(...)`, and project creation from the project list routes through `core.CreateProject(...)`. At this point, the TUI’s remaining direct use of the legacy service is overwhelmingly read-side rather than mutation-side.
- **Project reorder migration landed** — Project-detail `ctrl+j` / `ctrl+k` reorder actions now go through `core.ReorderProjectTask(taskID, delta)` rather than calling the legacy filename/subgroup-index reorder service directly from the TUI. The new core API resolves the project task by stable task ID and delegates internally, removing another clear direct legacy mutation seam from the UI layer.
- **Raw-payload trash-undo cleanup landed** — The remaining trash undo paths that still held pre-trash task payloads now call `core.RestoreTrashedTask(task, source)` instead of invoking the legacy raw-restore service method directly from the TUI. This keeps the transitional raw-payload restore behavior behind the core boundary while leaving a future redesign of trash undo semantics optional.
- **View-results / weekly-review mutation cleanup landed** — Aggregate task mutation surfaces now use the generic core task APIs for both list and project tasks. View results and weekly review state changes route through `core.UpdateTask(...)`, archive through `core.ArchiveTask(...)`, trash through `core.TrashTask(...)`, and view-results state-change undo now restores through `core.UpdateTask(...)` instead of legacy location-aware service methods.
- **Process Inbox TUI consolidation landed** — The TUI no longer keeps redundant Process Inbox copies of item arrays, indexes, or summary counters. Progress/completion rendering now re-reads summary and progress from the core-owned session, and Process Inbox bookkeeping no longer manually increments local counters after actions. The remaining TUI ownership is now mostly rendering, input/focus mechanics, mirrored current-draft presentation, and step-navigation UI behavior.
- **Process Inbox Slice C started** — `internal/core` now also owns typed final Process Inbox decision semantics via `CommitInboxDecision(sessionID, decision)`. Trash, done, someday, waiting, single-action refile, existing-project refile, and new-project creation+refile now commit through the core boundary, leaving the TUI primarily responsible for rendering, key/input mechanics, and step-navigation state.
- **Process Inbox Slice B started** — `internal/core` now also owns Process Inbox draft mutation via `UpdateInboxDraft(sessionID, patch)`, and the TUI now routes Process Inbox text/notes/waiting_on/tags/deadline/scheduled edits through that core session draft API. This moves the ephemeral draft much closer to the architecture target while still leaving step transitions and final action orchestration in the TUI for now.
- **Process Inbox Slice A landed** — `internal/core` now owns the first real Process Inbox session lifecycle seam: starting an inbox-processing session, exposing current item/progress/summary state, advancing via skip, and discarding the ephemeral session. The TUI now starts Process Inbox from `core.StartInboxSession()` and advances via `core.SkipInboxItem(...)`, while still keeping local working-copy draft mutation and final action orchestration in the TUI for now.
- **Refile/move workflow migration advanced again** — `internal/core` now handles more of the app’s location-changing task semantics via stable-ID `MoveTaskToList` / `MoveTaskToProject`, and the migration now includes Process Inbox route actions after they persist their working copy back to inbox. A temporary `ListProjectSummaries()` bridge was added so picker/new-project flows can still translate filename-based UI state into stable project IDs during extraction. Project -> different-project movement remains explicitly unimplemented for now, and Process Inbox itself is still only partially migrated rather than fully core-owned.
- **Project edit migration slice landed** — The project metadata edit view now saves through `core.UpdateProject(...)` instead of the legacy filename-based update path. The existing working-copy UX and rename/reload flow remain intact, which removes another self-contained legacy mutation island without forcing project list/detail rendering changes.
...
- **Project `waiting_on` metadata now persists through the core-backed update path** — The legacy `internal/service.UpdateProject(...)` adapter used by `core.UpdateProject(...)` now copies `Project.WaitingOn` alongside the other mutable project metadata fields. Clearing `waiting_on` through the project patch path now round-trips correctly without regressing rename behavior or subgroup/task preservation.
- **Project detail/edit loading now starts from stable project IDs in the TUI** — The first ID-first read-model cleanup slice landed for project screens. The TUI now tracks `activeProjectID`, opens project detail from project-list / weekly-review contexts by stable project ID, and loads both project detail and project edit through `core.GetProject(...)` instead of direct filename/store reads. Filenames still remain as derived metadata for reloads and display during the transition, but they are no longer the primary entry identity for these flows.
- **Project edit save/return now reloads detail by stable project ID** — The next project read-model cleanup slice removed another filename-first seam: after saving project edits from project detail, the TUI now returns by reloading the project through `core.GetProject(...)` using the stable project ID rather than re-entering by filename. Rename behavior still updates the derived filename metadata for display and source-aware rendering, but project identity no longer depends on the slug after save.
- **Aggregated project-task detail flows now carry stable project IDs** — `service.ViewTask` now includes `ProjectID` for project-backed rows, and view-results / weekly-review task-detail entry paths now seed project context with the stable project ID in addition to transitional filename metadata. This removes another class of aggregate-view → project-detail handoff that previously depended only on filenames.
- **Project-detail reload guards now key off stable project identity** — Follow-up cleanup replaced a couple of remaining `activeFilename != ""` checks with `activeProjectID != ""` in project-detail reload paths, and removed stale `activeFilename` suppression from task-detail save. Filename is still retained where genuinely needed for legacy source reconstruction, but reload eligibility now reflects the TUI’s ID-first project identity more accurately.
- **Subgroup-ID service adapters now exist below core** — The first subgroup-contract cleanup slice landed in the legacy service layer: rename/delete/add-task/move-between-subgroups now also have subgroup-ID-based helpers, and the core subgroup APIs now prefer those helpers instead of immediately translating subgroup IDs back into subgroup indexes for every operation. The persistence model is still unchanged and `SubgroupIx` remains available as a transitional bridge, but subgroup IDs now carry farther down the mutation path than before.
...
- **Weekly Review now preserves your place across refreshes** — Refreshing the review, mutating an item in waiting/someday, or returning from task detail no longer dumps you back to the first section. Weekly Review now distinguishes initial entry from in-review reloads, preserving the active section and per-section cursor positions while clamping them against updated bucket sizes.
- **Weekly Review now auto-advances when a section empties** — During in-review reloads, if the current section becomes empty after an action, Weekly Review now jumps to the next non-empty section instead of leaving you on a dead end. A short status message announces the section change, while the all-empty case remains stable and simply shows empty states.
