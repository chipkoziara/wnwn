...
- TUI now holds both the legacy `*service.Service` and the new `*core.Core` during the extraction. Migrated non-project paths already using core APIs include: task detail save for non-project tasks; archive/trash/restore in view-results and weekly review for non-project tasks; inbox/actions direct done/cancel/archive/trash actions for non-project tasks; in-place waiting/someday transitions for single-actions tasks; and the first non-Process-Inbox refile/move flows (refile to list, picker-based move to project, inbox waiting/someday routing). Project-detail operations now using core APIs include subgroup create/rename/delete, add-project-task, move-task-between-subgroups, project task done/cancel/archive/trash, in-place waiting/someday, project task detail save, and project metadata edit save. Remaining legacy areas are narrower: reorder-within-subgroup, some trash undo paths that still restore from raw task payload, Process Inbox, and the rest of the move/refile workflow family.
...
- **Refile/move workflow migration started** — `internal/core` now exposes the first stable-ID move/refile APIs (`MoveTaskToList`, `MoveTaskToProject`), and non-Process-Inbox inbox/actions refile flows have started using them. Inbox -> single-actions waiting/someday now also route through the new core move-to-list path because they are location-changing workflows, not just in-place state updates. Project -> different-project movement remains explicitly unimplemented for now, and Process Inbox still uses legacy movement paths.
