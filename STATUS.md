...
- **Core boundary scaffold** (`internal/core/`): first-pass client-facing facade over the legacy service/store stack.
  ...
  - Core mutation/session APIs now wrap the existing implementation for:
    ...
    - Process Inbox session lifecycle and draft mutation (`StartInboxSession`, `GetInboxSession`, `UpdateInboxDraft`, `SkipInboxItem`, `DiscardInboxSession`)
...
- TUI now holds both the legacy `*service.Service` and the new `*core.Core` during the extraction. Migrated non-project paths already using core APIs include: task detail save for non-project tasks; archive/trash/restore in view-results and weekly review for non-project tasks; inbox/actions direct done/cancel/archive/trash actions for non-project tasks; in-place waiting/someday transitions for single-actions tasks; the first non-Process-Inbox refile/move flows (refile to list, picker-based move to project, inbox waiting/someday routing); Process Inbox route movement after working-copy persistence back to inbox; and Process Inbox session start/advance lifecycle plus draft mutation for text/notes/waiting_on/tags/deadline/scheduled fields. Project-detail operations now using core APIs include subgroup create/rename/delete, add-project-task, move-task-between-subgroups, project task done/cancel/archive/trash, in-place waiting/someday, project task detail save, and project metadata edit save. Remaining legacy areas are narrower: reorder-within-subgroup, some trash undo paths that still restore from raw task payload, Process Inbox step/final-action ownership, and the rest of the move/refile workflow family.
